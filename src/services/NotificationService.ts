...
class NotificationService {
    initialize() {
        ...
        console.log('NotificationService: Initializing OneSignal with ID:', ONESIGNAL_APP_ID);
        ...
        OneSignal.Notifications.addEventListener('click', (event: any) => {
            ...
            const data = event.notification.additionalData as NotificationData | undefined;
            console.log('📋 Additional data:', data);

            if (data && (data.type === 'new_episode' || data.episode_url)) {
                console.log('🎵 New episode notification detected, navigating to player...');
        ...
        OneSignal.Notifications.addEventListener('foregroundWillDisplay', (event: any) => {
            ...
            console.log('⚠️ Note: Foreground notifications do NOT appear in system tray by default');
        });
        ...
        setTimeout(async () => {
            ...
            if (!userId || !token) {
                console.warn('⚠️ Device not registered with OneSignal. This is normal on emulators without Google Play Services.');
                console.warn('💡 Try testing on a real device or send a test notification from OneSignal Dashboard.');
            } else {
    ...
    navigateToPlayer(data: NotificationData) {
        ...
        const episode: any = { // Use 'any' or update interface to include id/audioUrl
            id: id,
